...
import es.cice.models.Amigo;
import es.cice.models.Publicacion;
import es.cice.models.Usuario;
...
public class WordManager {
    ...
    public Amigo getAmigo(String id_publicador){
        BDManager bdManager = new BDManager();
        String sql = "SELECT * FROM consolasadm WHERE id_consola="+id_publicador;
        ResultSet result = bdManager.execSelect(sql);
		...
		Amigo ami = new Amigo(nick, nombre, apellidos);
                return ami;
            }
        }catch(Exception e){
            System.out.println("error en el while");
